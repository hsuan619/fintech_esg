import streamlit as st

# 匯入 UI 模組
from ui import tab_pdf_to_md, tab_generate_prompt, tab_risk_assessment, tab_pdf_to_json, tab_manual_process

# 設定頁面配置 (必須是第一個 Streamlit 指令)
st.set_page_config(page_title="ESG 報告書漂綠稽核小幫手", layout="wide", page_icon="🌱")

# ==========================================
# Streamlit 網頁介面 (Web Interface)
# ==========================================

st.title("🌱 ESG 報告書漂綠稽核小幫手")
st.markdown("""
本工具提供從 **PDF 報告轉換** 到 **績效風險稽核** 的一站式流程。
請依序使用下方分頁功能：
""")

# 建立四個主要頁籤 (新增 "手動處理")
tab1, tab2, tab3, tab4, tab5 = st.tabs([
    "📄 1. 報告轉換 (PDF to MD)", 
    "🤖 2. 產生稽核 Prompt", 
    "🚀 3. 自動目標擷取 (PDF to JSON)", # 這是之前 tab_pdf_to_json 的位置，假設您有
    "🛠️ 4. 手動處理模式 (免 API)",      # [新增]
    "📊 5. 績效追蹤與風險評估"
])

# ------------------------------------------
# Tab 1: 報告轉換 (PDF -> Markdown)
# ------------------------------------------
with tab1:
    tab_pdf_to_md.render()

# ------------------------------------------
# Tab 2: 產生稽核 Prompt (基於 MD)
# ------------------------------------------
with tab2:
    tab_generate_prompt.render()

# ------------------------------------------
# Tab 3: 自動目標擷取 (PDF -> JSON, 需 API)
# ------------------------------------------
with tab3:
    # 假設您有這個模組，若沒有可省略
    if hasattr(tab_pdf_to_json, 'render'):
        tab_pdf_to_json.render()
    else:
        st.info("此功能模組尚未載入")

# ------------------------------------------
# Tab 4: 手動處理模式 (新增)
# ------------------------------------------
with tab4:
    tab_manual_process.render()

# ------------------------------------------
# Tab 5: 績效追蹤與風險評估
# ------------------------------------------
with tab5:
    tab_risk_assessment.render()